...
test("survey UI is bilingual, responsive and service worker cache is bumped",()=>{
  ...
  assert.match(worker,/BUILD_VERSION="2026-09-02-57"/);
});
...
test("Survey campaign UI supports persisted link QR PDFs and demographic bulk selection",()=>{
  assert.match(ui,/src="vendor\/qrcode\.min\.js\?v=2026-09-02-57"/);
  assert.match(ui,/src="vendor\/jspdf\.umd\.min\.js\?v=2026-09-02-57"/);
