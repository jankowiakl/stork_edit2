...
test("the protected public Survey keeps its existing language subsystem",()=>{
  ...
  assert.match(workerSource,/stork-edit2-shell-v2026-09-02-54/);
...
test("service worker updates bypass stale HTTP caches and take control without clearing user data",()=>{
  ...
  assert.match(indexSource,/serviceWorker\.addEventListener\("controllerchange"/);
  assert.match(indexSource,/serviceWorkerHadController&&!serviceWorkerReloading/);
