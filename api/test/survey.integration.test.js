...
test("real Survey API keeps a fixed photo set, protects ratings and applies the Top 10 threshold",{skip:!testDatabaseUrl,timeout:120000},async()=>{
  ...
  try{
    ...
    let result=await json("/api/surveys",{method:"POST",headers:adminHeaders,body:JSON.stringify(campaignBody)});assert.equal(result.response.status,201);const link=result.body.links[0],token=link.token,createdCampaignId=result.body.campaign.id;
    const storedLink=(await db.query("SELECT token_hash,token_ciphertext FROM survey_links WHERE campaign_id=$1",[createdCampaignId])).rows[0];
    assert.equal(storedLink.token_hash.length,64);assert.ok(storedLink.token_ciphertext);assert.notEqual(storedLink.token_ciphertext,token);assert.equal(storedLink.token_ciphertext.includes(token),false);
    const linkCountBefore=Number((await db.query("SELECT count(*) count FROM survey_links WHERE campaign_id=$1",[createdCampaignId])).rows[0].count);
    result=await json(`/api/surveys/${encodeURIComponent(createdCampaignId)}/links`,{headers:adminHeaders});assert.equal(result.response.status,200);assert.equal(result.body.links[0].url,link.url);assert.equal(result.body.links[0].status,"active");
    result=await json(`/api/surveys/${encodeURIComponent(createdCampaignId)}/links`,{headers:adminHeaders});assert.equal(result.response.status,200);assert.equal(Number((await db.query("SELECT count(*) count FROM survey_links WHERE campaign_id=$1",[createdCampaignId])).rows[0].count),linkCountBefore,"opening Edit campaign must not generate links");
    ...
    const campaignId=createdCampaignId,linkId=(await db.query("SELECT id FROM survey_links WHERE campaign_id=$1",[campaignId])).rows[0].id;
