...
export function normalizeAnnotationInput(input = {}, photo = {}) {
  ...
  const altitude = Number(photo.altitude_m ?? photo.Altitude_m);
  const elevation = Number(values.Elevation_m ?? photo.elevation_m ?? photo.Elevation_m);
  if (Number.isFinite(altitude) && Number.isFinite(elevation)) {
    values.Above_ground = Math.round((altitude - elevation) * 100) / 100;
    values.Height_class_100m = Math.max(0, Math.floor(values.Above_ground / 100) * 100);
  } else {
    values.Above_ground = null;
    values.Height_class_100m = null;
  }
  return values;
}
...
export function validateAnnotation(values, status = "draft") {
  ...
  for (const field of ANNOTATION_FIELDS) {
    ...
    if (field.options && !field.options.includes(value)) add(field.key, `${field.label} has an unsupported value.`);
